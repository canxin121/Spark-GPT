...
delete_bot = on_command("删除bot", priority=1, block=False)


@delete_bot.handle()
async def delete_bot_(
    matcher: Matcher, event: Event, state: T_State, args: Message = CommandArg()
):
...
async def delete_bot__(
    ...
    try:
        common_users.delete_bot(
            common_userinfo=common_userinfo, botinfo=BotInfo(nickname=bot_name)
        )
