...
def is_useable(event, mode=bard_persistor.mode):
    if bard_persistor.cookie:
        if mode == "black":
            try:
                if str(event.user_id) in bard_persistor.blacklist:
                    logger.warning(f"黑名单用户:{str(event.user_id)},跳过")
                    return False
            except:
                pass
            try:
                if str(event.group_id) in bard_persistor.blacklist:
            ...
            try:
                if str(event.user_id) in bard_persistor.whitelist:
                    return True
            except:
                pass
            try:
                if str(event.group_id) in bard_persistor.whitelist:
                    return True
            except:
                pass
        ...
        logger.warning("没有配置Bard的cookie,无法使用,跳过")
        return False
...
async def sendmsg(msg, matcher, event):
    if bard_persistor.pic_able is not None:
        if bard_persistor.pic_able == "True":
            pic = await md_to_pic(msg)
            if bard_persistor.url_able == "True":
        ...
        if len(msg) > bard_persistor.num_limit:
            pic = await md_to_pic(msg)
            if bard_persistor.url_able == "True":
                url = await get_url(msg)
                msgid_container = await matcher.send(
