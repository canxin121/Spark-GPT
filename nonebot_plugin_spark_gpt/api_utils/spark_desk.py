...
import copy
import json
from ..source_data.spark_desk_config import spark_desk_config


class SparkChat:
    def __init__(self,userdata):
        self.cookie = spark_desk_config.cookie
        self.fd = spark_desk_config.fd
        self.GtToken = spark_desk_config.GtToken
        self.sid = spark_desk_config.sid
        self.headers = {
                ...
                try:
                    async with session.post(url, data=payload) as response:
                        ...
                        if response_data["code"] == 0:
                            ...
                            raise Exception(
                                f"Failed to generate chat ID. Error code: {response_data['code']}"
                            )
