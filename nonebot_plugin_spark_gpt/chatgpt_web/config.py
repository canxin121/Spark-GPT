...
# 储存GPT_Web的持久数据,提供一键保存和加载功能
class GPT_WebPersistor(BaseModel):
    path_: str = str(Path()) + "/data/spark_gpt/gpt_web_data.json"
    session_token_path: str = str(Path()) + "/data/spark_gpt/gpt_web_session_token.txt"
    session_token: str = ""
    pic_able: str = None
    url_able: str = "True"
    num_limit: int = 350
    ...
    def __init__(self, **data):
        ...
        self.superusers = (
            get_config.gpt_web_superusers
            if hasattr(get_config, "gpt_web_superusers")
            else spark_persistor.superusers
        )
        self.blacklist = (
            get_config.gpt_web_blacklist
            if hasattr(get_config, "gpt_web_blacklist")
            else spark_persistor.blacklist
        )
        self.whitelist = (
            get_config.gpt_web_whitelist
            if hasattr(get_config, "gpt_web_whitelist")
            else spark_persistor.whitelist
        )
        self.mode = (
            get_config.gpt_web_mode
            if hasattr(get_config, "gpt_web_mode")
            and get_config.gpt_web_mode in ["white", "black"]
            else spark_persistor.mode
        )
        self.pic_able = getattr(get_config, "gpt_web_picable", spark_persistor.pic_able)
        self.url_able = getattr(get_config, "gpt_web_urlable", spark_persistor.url_able)
        self.num_limit = int(
            getattr(get_config, "gpt_web_limit", spark_persistor.num_limit)
        )
        self.model = getattr(get_config, "gpt_web_model", self.model)
        self.api_url = getattr(get_config, "gpt_web_api_url", self.api_url)
        self.proxy = getattr(get_config, "gpt_web_proxy", None)
        try:
            with open(self.session_token_path, "r") as f:
                self.session_token = f.read().strip()
        except:
            logger.error("gpt_web 的 session_token.txt文件有误，请检查")
        self.save()
...
gptweb_persistor = GPT_WebPersistor()
